...
# 管理 Vitest 配置文件 {#managing-vitest-config-file}

如果你正在使用 Vite，并且有 `vite.config` 文件，Vitest 会读取 Vite 配置并应用插件。如果你想使用不同的配置进行测试，或者你的主应用程序不特别依赖 Vite，你可以：
...
要配置 `vitest` 本身，请在 Vite 配置中添加 `test` 属性。如果你要从 `vite` 本身导入`defineConfig`，还需要在配置文件顶部使用 [三斜杠命令](https://www.typescriptlang.org/docs/handbook/triple-slash-directives.html#-reference-types-) 添加 Vitest 类型引用。

使用 `vite` 中的 `defineConfig`，你应该遵循如下配置：
