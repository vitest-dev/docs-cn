...
::: warning
Vitest 使用 Vite SSR 原始构建块来运行测试时具有[某些陷阱](https://vitejs.dev/guide/ssr.html#ssr-externals)。

1. 别名仅影响由 [inlined](#server-deps-inline) 模块直接使用 `import` 关键字导入的模块（默认情况下所有源代码都是内联的）。
2. Vitest 不支持别名 `require` 调用。
3. 如果您要为外部依赖项添加别名（例如，`react` -> `preact`），您可能需要为实际的`node_modules`包添加别名，以使其适用于外部依赖项。 [Yarn](https://classic.yarnpkg.com/en/docs/cli/add/#toc-yarn-add-alias) 和 [pnpm](https://pnpm.io/aliases/) 都支持别名 `npm:` 前缀。
:::
