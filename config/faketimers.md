...
仅当全局对象上存在 `Temporal` 时，才会对其进行模拟：原生支持（Node.js ≥ 26 默认支持，旧版本需通过 `--harmony-temporal ` 开启，或使用原生支持 `Temporal` 的浏览器），或通过全局安装的 polyfill 提供支持，例如 `import 'temporal-polyfill/global'`。
