...
允许通过 API 运行任意测试文件。适用于 [UI 模式](/guide/ui) 中能够运行代码的交互元素（以及其背后的服务端代码）。在浏览器模式下，此选项还会限制间接的代码执行，包括在服务器上解析并处理 `.snap` 文件，以及通过 [`cdp()`](/api/browser/context#cdp) 访问原始 Chrome DevTools 协议。
