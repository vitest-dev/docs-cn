...
另外，即使启用了此选项，被内联的 CommonJS 模块也不会经过 Vite 插件处理。由于 `require` 调用会离开模块运行器，mock 等依赖模块运行器的功能无法作用于这类模块。 有关配置替代方案，请参阅 [不完全支持 CommonJS 源码](/guide/common-errors#commonjs-source-code-is-not-fully-supported)。
:::
