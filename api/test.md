...
::: tip
在 Jest 中，`TestFunction` 也可以是 `(done: DoneCallback) => void` 类型。如果使用该形式，测试须在 `done` 被调用后才会结束。你可以改用 `async` 函数来实现相同的效果，更多内容请参阅 [迁移指南中 Done 回调章节](/guide/migration/jest#done-callback)。
:::
