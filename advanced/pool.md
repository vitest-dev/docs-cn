...
如果我们在不同 pools 中运行测试，可以使用 [`projects`](/guide/projects) 功能:
...
当运行新测试时， Vitest 会调用 `runTest` 。如果 `files` 为空，则不会调用它。第一个参数是一个数组 [TestSpecifications](/advanced/api/test-specification) 。在调用 `runTests` 之前，使用 [`sequencer`](/config/#sequence-sequencer) 对文件进行排序。有可能（但不太可能）有两个相同的文件，但它总是有一个不同的项目-这是通过 [`projects`](/guide/projects) 配置实现的。
