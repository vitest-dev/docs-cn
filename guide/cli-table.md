...
| `--ui`                                          | 启用 UI 模式                                                 |
| `--open`                                        | 自动打开用户界面（默认值: `!process.env.CI`）                |
...
| `--reporter <name>`                             | 指定报告器                                                   |
| `--outputFile <filename/-s>`                    | 如果还指定了支持报告器，则将测试结果写入文件，使用 cac 的点符号表示多个报告程序的单个输出结果（比如: --outputFile.tap=./tap.txt） |
...
| `--coverage.exclude <pattern>`                  | 覆盖范围中要排除的文件。使用多个扩展名时，可指定多次（默认情况下：请参阅 [`coverage.exclude`](/config/#coverage-exclude)） |
| `--coverage.extension <extension>`              | 包含在覆盖范围内的扩展名。使用多个扩展名时，可指定多次 (默认值: `[".js", ".cjs", ".mjs", ".ts", ".mts", ".cts", ".tsx", ".jsx", ".vue", ".svelte"]`） |
...
| `--coverage.reporter <name>`                    | 使用的报告器。更多信息请访问 [`coverage.reporter`](/config/#coverage-reporter)。（默认值: `["text", "html", "clover", "json"]`） |
...
| `--coverage.thresholds.autoUpdate`              | 更新阈值:  当前覆盖率高于配置的阈值时，将 "lines"、"functions"、"branches"和 "statements" 更新到配置文件（默认值: `false`） |
| `--coverage.thresholds.lines <number>`          | 针对代码行的覆盖度阈值设定，更多信息请参阅 [istanbuljs](https://github.com/istanbuljs/nyc#coverage-thresholds)。此选项不适用于自定义 providers |
| `--coverage.thresholds.functions <number>`      | 针对函数的覆盖度阈值设定，更多信息请参阅 [istanbuljs](https://github.com/istanbuljs/nyc#coverage-thresholds)。 此选项不适用于自定义 providers |
| `--coverage.thresholds.branches <number>`       | 针对 branches 的覆盖度阈值设定，更多信息请参阅 [istanbuljs](https://github.com/istanbuljs/nyc#coverage-thresholds)。 此选项不适用于自定义 providers |
| `--coverage.thresholds.statements <number>`     | 针对 statements 的覆盖度阈值设定，更多信息请参阅 [istanbuljs](https://github.com/istanbuljs/nyc#coverage-thresholds)。 此选项不适用于自定义 providers |
| `--coverage.ignoreClassMethods <name>`          | 覆盖时要忽略的类方法名称数组。更多信息请参阅 [istanbuljs](https://github.com/istanbuljs/nyc#ignoring-methods)。该选项仅适用于 istanbul providers（默认值: `[]`） |
| `--coverage.processingConcurrency <number>`     | 处理覆盖率结果时使用的并发限制。（默认最小值介于 20 和 CPU 数量之间） |
| `--coverage.customProviderModule <path>`        | 指定自定义覆盖范围提供程序模块的模块名称或路径。 更多信息请参阅 [自定义 providers 覆盖范围](/guide/coverage#custom-coverage-provider)。 此选项仅适用于自定义 providers |
| `--coverage.watermarks.statements <watermarks>` | 语句覆盖率高/低阈值，格式：`<high>,<low>`                    |
| `--coverage.watermarks.lines <watermarks>`      | 行覆盖率高/低阈值，格式：`<high>,<low>`                      |
| `--coverage.watermarks.branches <watermarks>`   | 分支覆盖率高/低阈值，格式：`<high>,<low>`                    |
| `--coverage.watermarks.functions <watermarks>`  | 函数覆盖率高/低阈值，格式：`<high>,<low>`                    |
...
| `--browser.name <name>`                         | 在特定浏览器中运行所有测试。某些浏览器只适用于特定的 providers (比如 `--browser.provider`）。更多信息请参阅 [`browser.name`](/config/#browser-name) |
...
| `--browser.provider <name>`                     | 用于运行浏览器测试的 Provider。某些浏览器只适用于特定的提供 Provider，可以是 "webdriverio"， "playwright"， 或自定义 provider。 更多信息请参阅 [`browser.provider`](/config/#browser-provider)（默认值: `"webdriverio"`） |
| `--browser.providerOptions <options>`           | 传递给浏览器 provider。 更多信息请参阅 [`browser.providerOptions`](/config/#browser-provideroptions) |
| `--browser.slowHijackESM`                       | 让 Vitest 在浏览器环境下使用其内部的模块解析机制来启用 API。比如 vi.mock and vi.spyOn. 更多信息请参阅 [`browser.slowHijackESM`](/config/#browser-slowhijackesm)（默认值: `false`） |
| `--browser.isolate`                             | 运行测试时显示 Vitest UI（默认值: `!process.env.CI`）        |
| `--browser.fileParallelism`                     | 是否所有测试文件都应并行运行。使用 `--browser.file-parallelism=false` 禁用 （默认值: 比如 `--file-parallelism`） |
...
| `--poolOptions.threads.useAtomics`              | 使用 Atomics 同步线程。这在某些情况下可以提高性能，但在较旧的 Node 版本中可能会导致 segfault |
...
| `--poolOptions.vmThreads.useAtomics`            | 使用 Atomics 同步线程。这在某些情况下可以提高性能，但在较旧的 Node 版本中可能会导致 segfault |
| `--poolOptions.vmThreads.memoryLimit <limit>`   | 虚拟线程池的内存限制。如果发现内存泄漏，请尝试调整该值       |
| `--poolOptions.forks.isolate`                   | 在 forks pool 中隔离测试 （默认值: `true`）                  |
| `--poolOptions.forks.singleFork`                | 单个子进程内运行测试 (默认值: `false`）                      |
| `--poolOptions.forks.maxForks <workers>`        | 运行测试的最大进程数                                         |
| `--poolOptions.forks.minForks <workers>`        | 运行测试的最小进程数                                         |
| `--poolOptions.vmForks.isolate`                 | 在 forks pool 中隔离测试 (默认值: `true`）                   |
| `--poolOptions.vmForks.singleFork`              | 在单个子进程内运行测试 (默认值: `false`）                    |
...
| `--sequence.shuffle.files`                      | 以随机顺序运行文件。如果启用此选项，长时间运行的测试将不会提前开始 |
| `--sequence.shuffle.tests`                      | 以随机方式运行测试（默认值: `false`）                        |
| `--sequence.concurrent`                         | 使测试并行运行（默认值: `false`）                            |
| `--sequence.seed <seed>`                        | 设置随机化种子。如果 --sequence.shuffle（随机序列）是`false`，则此选项无效。 详情参阅 ["Random Seed" page](https://en.wikipedia.org/wiki/Random_seed) 了解更多信息 |
| `--sequence.hooks <order>`                      | 更改钩子的执行顺序。 可接受的值有: "stack", "list" and "parallel". 通过 [`sequence.hooks`](/config/#sequence-hooks) 查看更多信息 （默认值: `"parallel"`） |
...
| `--slowTestThreshold <threshold>`               | 慢速测试的阈值（以毫秒为单位）（默认值: `300`）              |
...
| `--clearScreen`                                 | 在 watch 模式下重新运行测试时，清除终端屏幕（默认值: `true`） |
