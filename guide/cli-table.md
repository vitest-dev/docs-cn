...
| `--browser.slowHijackESM`                       | 让 Vitest 在浏览器环境下使用其内部的模块解析机制来启用 APIs。比如 vi.mock and vi.spyOn. 通过 [`browser.slowHijackESM`](https://vitest.dev/config/#browser-slowhijackesm) 查看更多信息 (默认值: `false`)                                   |
