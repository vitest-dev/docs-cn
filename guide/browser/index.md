...
Vitest 使用 [Vite dev server](/guide/#browser-support) 来运行您的测试，因此我们只支持 [`esbuild.target`](https://cn.vitejs.dev/config/shared-options.html#esbuild)选项（默认为 `esnext`）中指定的功能。
