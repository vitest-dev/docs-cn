...
::: info
所有提供的图像均使用 [Zammad](https://github.com/zammad/zammad) 仓库作为示例。
:::
...
::: warning
请注意，仅类型导入在运行时不执行，不显示总持续时间。它们也无法打开。
:::

如果另一个插件在转换期间注入模块导入，这些导入将在模块开始处以灰色显示（例如，通过 `import.meta.glob` 注入的模块）。它们也显示总时间并且可以进一步遍历。
