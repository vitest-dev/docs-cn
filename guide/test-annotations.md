...
::: warning
`annotate` 函数返回一个 Promise，因此如果你依赖它的结果，需要使用 await。不过，Vitest 也会在测试结束前自动等待任何未被 await 的注解完成。
:::

根据你使用的报告器不同，注解的显示方式也会有所不同。
...
`verbose` 报告器是唯一一个在测试通过时也会报告注解的终端报告器。
...
HTML 报告器以与 UI 相同的方式显示注解。你可以在调用注解的那一行看到它。目前，如果注解不是在测试文件中调用的，你将无法在 UI 中看到它。我们计划支持单独的测试摘要视图来展示这些注解。
...
`junit` 报告器将注解列在测试用例的 `properties` 标签内。JUnit 报告器会忽略所有附件，只打印类型和消息。
...
`github-actions` 报告器默认将注解打印为 [notice 消息](https://docs.github.com/en/actions/writing-workflows/choosing-what-your-workflow-does/workflow-commands-for-github-actions#setting-a-notice-message)。你可以通过传递第二个参数来配置 `type`，可选值为 `notice`、`warning` 或 `error`。如果类型不是这些值之一，Vitest 将以 notice 形式显示消息。
...
`tap` 和 `tap-flat` 报告器将注解打印为新行上以 `#` 符号开头的诊断消息。它们会忽略所有附件，只打印类型和消息：
