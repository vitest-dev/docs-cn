...
## 从 Vitest 0.34.6 迁移 {#migrating-from-vitest-0-34-6}
...
### 快照更新 [#3961](https://github.com/vitest-dev/vitest/pull/3961) {#snapshots-update-3961}
...
### 覆盖率的变化 [#4265](https://github.com/vitest-dev/vitest/pull/4265), [#4442](https://github.com/vitest-dev/vitest/pull/4442) {#changes-to-coverage-4265-4442}
...
### 模拟定时器 [#3925](https://github.com/vitest-dev/vitest/pull/3925) {#timer-mocks-3925}
...
### 访问模拟 Promise 的返回值 {#accessing-the-return-values-of-a-mocked-promise}

Jest 和 Vitest 都将所有模拟调用的结果存储在 [`mock.results`](/api/mock.html#mock-results) 数组中，其中每次调用的返回值存储在 `value` 属性中。
然而，在模拟或监听 Promise 时（例如：使用 `mockResolvedValue`），Jest 的 `value` 属性会保持为 Promise 对象，而 Vitest 则会在 Promise 解析后将其转换为已解析的值。
