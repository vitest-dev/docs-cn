...
前面的页面介绍了 Vitest API：断言、模拟、快照和测试生命周期钩子。本章重点介绍如何将这些工具应用到实际代码中，包括如何确定测试内容、如何组织测试结构，以及如何在项目增长时有效组织测试文件。
...
对于具有广泛有效输入范围的函数，只靠手动挑选边界情况，覆盖面终究有限。**基于属性的测试** 是一种技术，你描述任何输入都应该成立的 _属性_，测试框架会生成数百个随机输入来尝试找到破坏这些属性的情况。
