...
你能做的最重要的一件事就是给 AI 提供足够的上下文，让它理解要测试什么。

首先从源文件本身开始。AI 需要看到具体的实现，而不仅仅是函数功能的描述。提供完整的文件，或者至少提供你要测试的函数及其导入和类型。

共享同一项目中现有的测试文件。这有助于 AI 遵循你的约定：是使用 `test` 还是 `it`、如何组织 `describe` 块、是偏好 `test.extend` fixtrue 还是 `beforeEach`，以及如何命名测试。AI 工具擅长遵循约定，但它们需要有可遵循的约定。
...
- 提及你想要你希望使用的特定 Vitest 功能。“使用 `toMatchInlineSnapshot` 处理错误信息” 或 “使用 `test.each` 处理不同的货币格式”，引导 AI 使用正确的工具，而不是让它退回到重复的复制粘贴测试。
...
AI 工具倾向于生成成功路径测试而跳过困难情况。在审查生成的测试后，问问自己：空输入会发生什么？输入 `null` 或 `undefined` 呢？网络请求失败怎么办？列表为空的情况呢？
